		...
		<div class="grow max-w-sm sm:max-w-md md:max-w-lg lg:max-w-xl xl:max-w-2xl transition-all duration-300">
			...
			<Form @submit="onSubmit" :validation-schema="schema">
				...
				<button @click="onSubmit()"
					...
					<div
						class="absolute h-0 w-0 opacity-0 bg-BN_PRIMARY -z-[1] rounded-2xl group-hover:h-12 group-hover:w-full sm:group-hover:w-52 group-hover:opacity-100 transition-all duration-500">
					</div>
					Submit
					<i
						class="fa-solid fa-paper-plane ml-2 text-BN_PRIMARY group-hover:text-BN_FONT_WHITE group-hover:ml-4 transition-all duration-500"></i>
				</button>
				...
				<BN_alert :show="type == 'warning'" color="rgb(220, 38, 68)" icon="fa-solid fa-triangle-exclamation"
					class="absolute -mt-12">
					<b>Error sending message!</b> <br />
					This is not your fault, we are probably working on it!
				</BN_alert>
				<BN_alert :show="type == 'success'" color="rgb(12, 173, 134)" icon="fa-solid fa-circle-check"
					class="absolute -mt-12">
